...
zokou(
  ...
  async (dest, origine, msg) => {
    ...
    try {
      if (!arg || arg.length === 0) {
        return repondre("*Please provide a number in the format: 25576311......*");
      }

      await repondre("*Please wait RAHEEM-XMD... Generating pair code*");

      const encodedNumber = encodeURIComponent(arg.join(" "));
      const apiUrl = `https://Raheem-cm.onrender.com/code?number=${encodedNumber}`;
